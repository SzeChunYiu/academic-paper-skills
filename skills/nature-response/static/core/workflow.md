...
17. Mark a claimed change `VERIFIED_DONE` only after matching it to supplied revised manuscript text, analysis output, figure/table content, or another inspectable artifact and checking that the warrant entails the exact atomic proposition. Treat an unsupported author report as `REPORTED_DONE_UNVERIFIED`. A locator alone does not establish closure.
