...
def test_contract_exists_and_states_the_governing_rule() -> None:
    text = _flat(CONTRACT)
...
def test_contract_forbids_relabelling_as_a_remedy_when_the_label_overstates() -> None:
    text = _flat(CONTRACT)
    assert "relabelling is not available" in text or "relabelling is not an available" in text


def test_contract_protects_the_disclosure_and_refuses_detector_targeting() -> None:
    text = _flat(CONTRACT)
    assert "that disclosure stays" in text
    assert "machine-text detector" in text


def test_contract_states_its_own_limits() -> None:
    text = _flat(CONTRACT)
...
def test_pipeline_routes_the_contract_without_growing_the_always_load_set() -> None:
...
def test_pipeline_exposes_all_three_executables() -> None:
    manifest = PIPELINE_MANIFEST.read_text(encoding="utf-8")
    for script in (
...
def test_rules_carry_a_source_url_and_operative_quotation() -> None:
...
def test_rules_distinguish_named_from_construed_content_types() -> None:
...
def test_no_rule_penalises_disclosed_ai_assistance() -> None:
    doc = json.loads(RULES.read_text(encoding="utf-8"))
...
def test_detector_reads_a_research_body_as_primary_research() -> None:
...
def test_detector_fires_on_a_survey_title_alone() -> None:
...
def test_detector_fires_on_an_explicit_primary_research_negation() -> None:
...
def test_detector_reads_a_synthesis_body_as_synthesis() -> None:
    rec = detector.detect(SYNTHESIS_BODY, "tex", {"title": "A Survey of Things"})
    assert rec["synthesis_indicators"]


def test_composition_is_not_read_as_a_position_paper() -> None:
...
def test_descriptively_named_results_sections_are_not_penalised() -> None:
...
def test_markdown_headings_are_found_in_a_mixed_format_body() -> None:
...
def test_gate_blocks_a_perspective_in_cs_without_peer_review() -> None:
    res = gate.evaluate(_rules_doc(), _decl(declared_content_type="perspective"),
                        _detection([]), TODAY, 120)
    assert res["verdict"] == "BLOCK"


def test_gate_passes_the_same_paper_once_peer_reviewed() -> None:
...
def test_gate_passes_ordinary_research_in_cs() -> None:
    res = gate.evaluate(_rules_doc(), _decl(), _detection([]), TODAY, 120)
    assert res["verdict"] == "PASS"


def test_rule_is_out_of_scope_for_a_mathematics_primary() -> None:
    res = gate.evaluate(_rules_doc(),
...
def test_cross_list_into_cs_does_not_escape_the_cs_practice() -> None:
    res = gate.evaluate(_rules_doc(),
...
def test_evidence_of_synthesis_triggers_even_when_the_author_declares_research() -> None:
    res = gate.evaluate(_rules_doc(), _decl(),
                        _detection(["declared_review_or_position_label"]), TODAY, 120)
    assert res["verdict"] == "BLOCK"


def test_unset_category_cannot_be_evaluated_and_is_not_a_pass() -> None:
    res = gate.evaluate(_rules_doc(), _decl(primary_category="AUTHOR_TO_SET"),
                        _detection([]), TODAY, 120)
    assert res["verdict"] == "CANNOT_EVALUATE"


def test_missing_detection_cannot_be_evaluated() -> None:
    res = gate.evaluate(_rules_doc(), _decl(), None, TODAY, 120)
    assert res["verdict"] == "CANNOT_EVALUATE"


def test_stale_ruleset_refuses_to_certify() -> None:
...
def test_overstating_label_blocks_and_refuses_to_offer_a_relabel() -> None:
    res = gate.evaluate(_rules_doc(), _decl(),
...
def test_understating_label_is_reported_as_correctable() -> None:
    res = gate.evaluate(_rules_doc(), _decl(),
...
def test_pass_is_not_described_as_a_prediction() -> None:
    res = gate.evaluate(_rules_doc(), _decl(), _detection([]), TODAY, 120)
...
def test_missing_reference_apparatus_is_an_error() -> None:
...
def test_a_paper_with_a_bibliography_is_clean() -> None:
    res = integrity.audit(RESEARCH_BODY + "\n\\printbibliography\n",
                          ["@article{smith2020, title={x}}"], None)
    assert res["error_count"] == 0


def test_setcitestyle_is_not_read_as_a_fabricated_reference() -> None:
...
def test_nocite_star_is_not_read_as_a_fabricated_reference() -> None:
    body = RESEARCH_BODY + "\n\\nocite{*}\n\\printbibliography\n"
    res = integrity.audit(body, ["@article{smith2020, title={x}}"], None)
    assert not any(f["code"] == "dangling_citation_keys" for f in res["findings"])


def test_a_genuinely_dangling_key_is_still_caught() -> None:
    body = RESEARCH_BODY.replace("smith2020", "ghost1999") + "\n\\printbibliography\n"
    res = integrity.audit(body, ["@article{smith2020, title={x}}"], None)
    assert any(f["code"] == "dangling_citation_keys" for f in res["findings"])


def test_assistant_voice_is_an_error() -> None:
...
def test_preamble_placeholder_is_a_warning_not_an_error() -> None:
    body = ("\\documentclass{article}\n\\def\\openreview{XXXX}\n\\begin{document}\n"
...
def test_body_placeholder_is_an_error() -> None:
    body = ("\\documentclass{article}\n\\begin{document}\n\\section{Results}\n"
...
def test_broken_cross_reference_is_caught() -> None:
...
def test_disclosure_divergence_is_informational_and_never_an_error() -> None:
...
def test_audit_declares_that_style_is_out_of_scope() -> None:
