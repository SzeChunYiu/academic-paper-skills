#!/usr/bin/env python3
"""Fail-closed validator for research-integrity ledgers.

The validator has two layers:
1. deterministic structural/release checks over a JSON ledger;
2. optional live bibliographic/status checks for DOI/PMID sources.

It intentionally does not treat model self-report as verification and it does not
claim that registry existence proves semantic support. Claim-to-source support
must be represented by evidence receipts produced from inspected source content.
"""
from __future__ import annotations

import argparse
import json
import re
import sys
import unicodedata
import urllib.error
import urllib.parse
import urllib.request
from collections import Counter
from difflib import SequenceMatcher
from pathlib import Path
from typing import Any, Iterable
from xml.etree import ElementTree

ALLOWED_RELEASE = {"VERIFIED", "BOUNDED_INFERENCE", "COHERENT_DEFINITION", "NOT_APPLICABLE"}
FAIL_CLOSED = {"SUPPORTED_INTERNAL", "UNRESOLVED", "CONTRADICTED", "BLOCKED", "NOT_ASSESSABLE"}
SUPPORT_PASS = {"ENTAILS", "BOUNDS"}
SOURCE_WARRANTS = {"source", "literature"}
INTERNAL_WARRANTS = {"author_data", "analysis", "proof", "method_record"}
VERIFICATION_METHODS = {
    "registry_lookup",
    "publisher_or_primary_record",
    "fulltext_span_check",
    "deterministic_recompute",
    "deterministic_derivation",
    "human_review",
    "independent_model_with_retrieved_source",
    "authoritative_project_record",
}
SELF_ATTEST_METHODS = {"model_self_report", "authoring_model_judgment", "title_only", "metadata_only"}
HIGH_RISK_CLASSES = {
    "causal",
    "clinical_or_safety",
    "novelty_or_priority",
    "quantitative_result",
    "legal_or_policy",
    "availability_or_compliance",
}
STATUS_BLOCKING = {"RETRACTED", "WITHDRAWN"}
STATUS_WARNING = {"CORRECTED", "EXPRESSION_OF_CONCERN", "UNKNOWN"}


def norm_text(value: str) -> str:
    value = unicodedata.normalize("NFKC", value or "").casefold()
    value = re.sub(r"[^\w]+", " ", value, flags=re.UNICODE)
    return " ".join(value.split())


def title_similarity(a: str, b: str) -> float:
    aa, bb = norm_text(a), norm_text(b)
    if not aa or not bb:
        return 0.0
    if aa == bb:
        return 1.0
    return SequenceMatcher(a=aa, b=bb).ratio()


def get_identifier(source: dict[str, Any], *schemes: str) -> str | None:
    wanted = {s.casefold() for s in schemes}
    for item in source.get("identifiers", []):
        if str(item.get("scheme", "")).casefold() in wanted:
            value = str(item.get("value", "")).strip()
            if value:
                return value.removeprefix("https://doi.org/").removeprefix("http://doi.org/")
    return None


def request_json(url: str, timeout: float, user_agent: str) -> dict[str, Any]:
    request = urllib.request.Request(url, headers={"User-Agent": user_agent, "Accept": "application/json"})
    with urllib.request.urlopen(request, timeout=timeout) as response:  # noqa: S310 - fixed scholarly endpoints
        return json.load(response)


def crossref_lookup(doi: str, *, timeout: float, user_agent: str, mailto: str | None) -> dict[str, Any]:
    encoded = urllib.parse.quote(doi, safe="")
    suffix = f"?mailto={urllib.parse.quote(mailto)}" if mailto else ""
    payload = request_json(f"https://api.crossref.org/works/{encoded}{suffix}", timeout, user_agent)
    message = payload.get("message", {})
    title = (message.get("title") or [""])[0]
    authors = [" ".join(filter(None, [a.get("given", ""), a.get("family", "")])).strip() for a in message.get("author", [])]
    issued = message.get("issued", {}).get("date-parts", [[None]])
    year = issued[0][0] if issued and issued[0] else None
    updates = message.get("updated-by") or []
    update_types = [norm_text(str(x.get("type", ""))) for x in updates if isinstance(x, dict)]
    status = "ACTIVE"
    if any("retract" in x or "withdraw" in x for x in update_types):
        status = "RETRACTED"
    elif any("expression of concern" in x for x in update_types):
        status = "EXPRESSION_OF_CONCERN"
    elif any("correct" in x or "update" in x for x in update_types):
        status = "CORRECTED"
    return {
        "provider": "crossref",
        "found": True,
        "doi": message.get("DOI", doi),
        "title": title,
        "authors": authors,
        "year": year,
        "status": status,
        "raw_update_types": update_types,
    }


def datacite_lookup(doi: str, *, timeout: float, user_agent: str) -> dict[str, Any]:
    encoded = urllib.parse.quote(doi, safe="")
    payload = request_json(f"https://api.datacite.org/dois/{encoded}", timeout, user_agent)
    attrs = payload.get("data", {}).get("attributes", {})
    titles = attrs.get("titles") or []
    creators = attrs.get("creators") or []
    return {
        "provider": "datacite",
        "found": True,
        "doi": attrs.get("doi", doi),
        "title": (titles[0].get("title") if titles else "") or "",
        "authors": [c.get("name", "") for c in creators],
        "year": attrs.get("publicationYear"),
        "status": "ACTIVE" if attrs.get("isActive", True) else "UNKNOWN",
    }


def openalex_status(doi: str, *, timeout: float, user_agent: str) -> dict[str, Any]:
    params = urllib.parse.urlencode({"filter": f"doi:https://doi.org/{doi}", "per_page": 1})
    payload = request_json(f"https://api.openalex.org/works?{params}", timeout, user_agent)
    results = payload.get("results") or []
    if not results:
        return {"provider": "openalex", "found": False, "status": "UNKNOWN"}
    work = results[0]
    return {
        "provider": "openalex",
        "found": True,
        "doi": (work.get("doi") or "").removeprefix("https://doi.org/"),
        "title": work.get("title") or "",
        "year": work.get("publication_year"),
        "status": "RETRACTED" if work.get("is_retracted") else "ACTIVE",
    }


def pubmed_lookup(pmid: str, *, timeout: float, user_agent: str) -> dict[str, Any]:
    params = urllib.parse.urlencode({"db": "pubmed", "id": pmid, "retmode": "xml"})
    request = urllib.request.Request(
        f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?{params}",
        headers={"User-Agent": user_agent},
    )
    with urllib.request.urlopen(request, timeout=timeout) as response:  # noqa: S310
        root = ElementTree.parse(response).getroot()
    article = root.find(".//PubmedArticle")
    if article is None:
        return {"provider": "pubmed", "found": False, "status": "UNKNOWN"}
    title_node = article.find(".//ArticleTitle")
    title = "".join(title_node.itertext()) if title_node is not None else ""
    pub_types = ["".join(x.itertext()) for x in article.findall(".//PublicationType")]
    status = "RETRACTED" if any(norm_text(x) == "retracted publication" for x in pub_types) else "ACTIVE"
    year_node = article.find(".//PubDate/Year")
    authors = []
    for author in article.findall(".//Author"):
        family = (author.findtext("LastName") or "").strip()
        given = (author.findtext("ForeName") or "").strip()
        name = " ".join(x for x in (given, family) if x)
        if name:
            authors.append(name)
    return {
        "provider": "pubmed",
        "found": True,
        "pmid": pmid,
        "title": title,
        "authors": authors,
        "year": int(year_node.text) if year_node is not None and (year_node.text or "").isdigit() else None,
        "status": status,
        "publication_types": pub_types,
    }


def compare_identity(source: dict[str, Any], record: dict[str, Any]) -> tuple[bool, list[str]]:
    problems: list[str] = []
    expected = source.get("bibliographic", {})
    expected_title = str(expected.get("title", "")).strip()
    if expected_title:
        score = title_similarity(expected_title, str(record.get("title", "")))
        if score < 0.95:
            problems.append(f"title mismatch (similarity={score:.3f})")
    expected_year = expected.get("year")
    actual_year = record.get("year")
    if expected_year and actual_year and int(expected_year) != int(actual_year):
        problems.append(f"year mismatch ({expected_year} != {actual_year})")
    expected_authors = [norm_text(str(x)) for x in expected.get("authors", []) if str(x).strip()]
    actual_authors = [norm_text(str(x)) for x in record.get("authors", []) if str(x).strip()]
    if expected_authors and actual_authors:
        expected_family = expected_authors[0].split()[-1]
        actual_family = actual_authors[0].split()[-1]
        if expected_family != actual_family:
            problems.append(f"first-author mismatch ({expected_family} != {actual_family})")
    return not problems, problems


def live_verify_source(source: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    doi = get_identifier(source, "doi", "datacite_doi")
    pmid = get_identifier(source, "pmid")
    records: list[dict[str, Any]] = []
    errors: list[str] = []
    if doi:
        try:
            records.append(crossref_lookup(doi, timeout=args.timeout, user_agent=args.user_agent, mailto=args.mailto))
        except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, ValueError, json.JSONDecodeError) as exc:
            errors.append(f"crossref: {exc}")
            try:
                records.append(datacite_lookup(doi, timeout=args.timeout, user_agent=args.user_agent))
            except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, ValueError, json.JSONDecodeError) as dc_exc:
                errors.append(f"datacite: {dc_exc}")
        try:
            records.append(openalex_status(doi, timeout=args.timeout, user_agent=args.user_agent))
        except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, ValueError, json.JSONDecodeError) as exc:
            errors.append(f"openalex: {exc}")
    elif pmid:
        try:
            records.append(pubmed_lookup(pmid, timeout=args.timeout, user_agent=args.user_agent))
        except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, ValueError, ElementTree.ParseError) as exc:
            errors.append(f"pubmed: {exc}")
    else:
        return {"source_id": source.get("source_id"), "verifiable_online": False, "records": [], "errors": []}

    found = [r for r in records if r.get("found")]
    identity_matches: list[dict[str, Any]] = []
    for record in found:
        ok, problems = compare_identity(source, record)
        identity_matches.append({"provider": record.get("provider"), "match": ok, "problems": problems})
    statuses = {str(r.get("status", "UNKNOWN")) for r in found}
    return {
        "source_id": source.get("source_id"),
        "verifiable_online": True,
        "records": records,
        "identity_matches": identity_matches,
        "status": (
            "RETRACTED" if "RETRACTED" in statuses else
            "CORRECTED" if "CORRECTED" in statuses else
            "EXPRESSION_OF_CONCERN" if "EXPRESSION_OF_CONCERN" in statuses else
            "ACTIVE" if "ACTIVE" in statuses else "UNKNOWN"
        ),
        "errors": errors,
    }


def require_keys(item: dict[str, Any], keys: Iterable[str], prefix: str, errors: list[str]) -> None:
    for key in keys:
        if key not in item or item[key] in (None, "", []):
            errors.append(f"{prefix}: missing required field {key}")


def validate_ledger(ledger: dict[str, Any], *, live: bool, args: argparse.Namespace) -> dict[str, Any]:
    errors: list[str] = []
    warnings: list[str] = []
    require_keys(ledger, ("schema_version", "manuscript_id", "claims", "sources", "evidence_receipts", "citation_usages"), "ledger", errors)

    sources = {str(s.get("source_id")): s for s in ledger.get("sources", []) if s.get("source_id")}
    claims = {str(c.get("claim_id")): c for c in ledger.get("claims", []) if c.get("claim_id")}
    receipts = {str(r.get("receipt_id")): r for r in ledger.get("evidence_receipts", []) if r.get("receipt_id")}
    authoring_agent = str(ledger.get("authoring_agent_id", "")).strip()
    release = ledger.get("release", {})
    requested = str(release.get("requested_state", "draft"))
    release_requested = requested in {"submission_ready", "publication_ready", "simulated_publication_ready_for_target"}

    for label, items, key in (("source", ledger.get("sources", []), "source_id"), ("claim", ledger.get("claims", []), "claim_id"), ("receipt", ledger.get("evidence_receipts", []), "receipt_id")):
        ids = [str(x.get(key, "")) for x in items]
        for duplicate, count in Counter(ids).items():
            if duplicate and count > 1:
                errors.append(f"duplicate {label} id: {duplicate}")

    live_results: list[dict[str, Any]] = []
    for sid, source in sources.items():
        require_keys(source, ("source_id", "source_type", "identifiers", "bibliographic"), f"source {sid}", errors)
        identifiers = source.get("identifiers", [])
        if not any(str(x.get("value", "")).strip() for x in identifiers if isinstance(x, dict)):
            errors.append(f"source {sid}: no stable or explicit identifier")
        bibliographic = source.get("bibliographic", {})
        require_keys(bibliographic, ("title",), f"source {sid}.bibliographic", errors)
        declared_status = str(source.get("declared_publication_status", "UNKNOWN"))
        if declared_status in STATUS_BLOCKING:
            errors.append(f"source {sid}: publication status is {declared_status}")
        elif declared_status in STATUS_WARNING:
            message = f"source {sid}: publication status is {declared_status}; dependent claims require explicit adjudication"
            if release_requested and source.get("status_adjudication", {}).get("status") != "PASS":
                errors.append(message)
            else:
                warnings.append(message)

        stored_identity = source.get("identity_checks", [])
        stored_status = source.get("status_checks", [])
        for check in [*stored_identity, *stored_status]:
            method = str(check.get("verification_method", ""))
            if method in SELF_ATTEST_METHODS or method not in VERIFICATION_METHODS:
                errors.append(f"source {sid}: untrusted source-check verification_method {method!r}")
        stored_identity_pass = any(str(x.get("status")) == "MATCH" for x in stored_identity)
        stored_status_pass = any(str(x.get("status")) in {"ACTIVE", "CORRECTED"} for x in stored_status)

        live_verified_identity = False
        live_verified_status = False
        if live:
            result = live_verify_source(source, args)
            live_results.append(result)
            if result.get("verifiable_online"):
                found = [r for r in result.get("records", []) if r.get("found")]
                if not found:
                    errors.append(f"source {sid}: identifier not resolved by configured live registries")
                live_verified_identity = bool(result.get("identity_matches")) and any(x.get("match") for x in result["identity_matches"])
                if result.get("identity_matches") and not live_verified_identity:
                    problems = "; ".join(", ".join(x.get("problems", [])) for x in result["identity_matches"])
                    errors.append(f"source {sid}: live metadata mismatch: {problems}")
                live_status = str(result.get("status", "UNKNOWN"))
                live_verified_status = live_status in {"ACTIVE", "CORRECTED"}
                if live_status in STATUS_BLOCKING:
                    errors.append(f"source {sid}: live status is {live_status}")
                elif live_status in STATUS_WARNING:
                    message = f"source {sid}: live status is {live_status}; requires explicit adjudication"
                    if release_requested and source.get("status_adjudication", {}).get("status") != "PASS":
                        errors.append(message)
                    else:
                        warnings.append(message)

        if release_requested and not (live_verified_identity or stored_identity_pass):
            errors.append(f"source {sid}: release requires a resolved identity check")
        if release_requested and not (live_verified_status or stored_status_pass):
            errors.append(f"source {sid}: release requires a current publication-status check")

    for rid, receipt in receipts.items():
        require_keys(receipt, ("receipt_id", "claim_id", "warrant_type", "verification_method", "support_status"), f"receipt {rid}", errors)
        cid = str(receipt.get("claim_id", ""))
        if cid not in claims:
            errors.append(f"receipt {rid}: unknown claim_id {cid}")
        method = str(receipt.get("verification_method", ""))
        if method in SELF_ATTEST_METHODS or method not in VERIFICATION_METHODS:
            errors.append(f"receipt {rid}: untrusted verification_method {method!r}")
        warrant_type = str(receipt.get("warrant_type", ""))
        if warrant_type in SOURCE_WARRANTS:
            sid = str(receipt.get("source_id", ""))
            if sid not in sources:
                errors.append(f"receipt {rid}: source warrant points to unknown source {sid}")
            if not str(receipt.get("locator", "")).strip():
                errors.append(f"receipt {rid}: source warrant requires an exact locator")
            if not str(receipt.get("evidence_fingerprint", "")).startswith("sha256:"):
                errors.append(f"receipt {rid}: source warrant requires sha256 evidence_fingerprint")
        elif warrant_type in INTERNAL_WARRANTS:
            if not str(receipt.get("artifact_pointer", "")).strip():
                errors.append(f"receipt {rid}: {warrant_type} warrant requires artifact_pointer")
        elif warrant_type not in {"definition", "not_applicable"}:
            errors.append(f"receipt {rid}: unknown warrant_type {warrant_type!r}")

    receipt_by_claim: dict[str, list[dict[str, Any]]] = {}
    for receipt in receipts.values():
        receipt_by_claim.setdefault(str(receipt.get("claim_id", "")), []).append(receipt)

    for cid, claim in claims.items():
        require_keys(claim, ("claim_id", "location", "text", "claim_class", "risk", "release_status", "independent_check"), f"claim {cid}", errors)
        release_status = str(claim.get("release_status", ""))
        if release_status in FAIL_CLOSED or release_status not in ALLOWED_RELEASE:
            errors.append(f"claim {cid}: non-closing release_status {release_status!r}")
        claim_receipts = receipt_by_claim.get(cid, [])
        if release_status != "NOT_APPLICABLE" and not claim_receipts:
            errors.append(f"claim {cid}: no evidence receipt")
        if claim_receipts and not any(str(r.get("support_status")) in SUPPORT_PASS for r in claim_receipts):
            errors.append(f"claim {cid}: no receipt with ENTAILS/BOUNDS support")
        if any(str(r.get("support_status")) == "CONTRADICTS" for r in claim_receipts):
            errors.append(f"claim {cid}: contradictory evidence receipt present")

        independent = claim.get("independent_check", {})
        if independent.get("status") != "PASS":
            errors.append(f"claim {cid}: independent_check must PASS")
        verifier = str(independent.get("verifier_id", "")).strip()
        if not verifier:
            errors.append(f"claim {cid}: independent_check missing verifier_id")
        elif authoring_agent and verifier == authoring_agent:
            errors.append(f"claim {cid}: independent verifier must differ from authoring agent")

        risk = str(claim.get("risk", ""))
        claim_class = str(claim.get("claim_class", ""))
        is_high = risk == "high" or claim_class in HIGH_RISK_CLASSES
        if is_high:
            counter = claim.get("counterevidence_search", {})
            if counter.get("status") not in {"DONE", "NOT_APPLICABLE"}:
                errors.append(f"claim {cid}: high-risk claim requires counterevidence_search DONE/NOT_APPLICABLE")
            if claim_class in {"causal", "clinical_or_safety", "novelty_or_priority", "legal_or_policy"}:
                source_receipts = [r for r in claim_receipts if str(r.get("warrant_type")) in SOURCE_WARRANTS]
                if not source_receipts:
                    errors.append(f"claim {cid}: high-risk external claim requires source evidence")

    for index, citation in enumerate(ledger.get("citation_usages", []), 1):
        prefix = f"citation_usage[{index}]"
        require_keys(citation, ("citation_id", "source_id", "location", "claim_ids"), prefix, errors)
        sid = str(citation.get("source_id", ""))
        if sid not in sources:
            errors.append(f"{prefix}: unknown source_id {sid}")
        claim_ids = [str(x) for x in citation.get("claim_ids", [])]
        if not claim_ids:
            errors.append(f"{prefix}: citation is not mapped to any atomic claim")
        for cid in claim_ids:
            if cid not in claims:
                errors.append(f"{prefix}: unknown claim_id {cid}")
            elif not any(str(r.get("source_id", "")) == sid for r in receipt_by_claim.get(cid, [])):
                errors.append(f"{prefix}: source {sid} has no evidence receipt for claim {cid}")

    cited_source_ids = {str(c.get("source_id", "")) for c in ledger.get("citation_usages", [])}
    for sid, source in sources.items():
        if source.get("bibliography_only"):
            continue
        if sid not in cited_source_ids:
            warnings.append(f"source {sid}: present in source registry but unused in citation_usages")

    decision = "BLOCKED" if errors else "PASS"
    return {
        "decision": decision,
        "error_count": len(errors),
        "warning_count": len(warnings),
        "errors": errors,
        "warnings": warnings,
        "live_source_checks": live_results,
        "summary": {
            "claims": len(claims),
            "sources": len(sources),
            "evidence_receipts": len(receipts),
            "citation_usages": len(ledger.get("citation_usages", [])),
        },
    }


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Validate a fail-closed research-integrity ledger.")
    parser.add_argument("ledger", type=Path, help="Path to research-integrity ledger JSON")
    parser.add_argument("--online", action="store_true", help="Refresh DOI/PMID identity and retraction/status checks")
    parser.add_argument("--mailto", help="Contact email for Crossref polite-pool requests")
    parser.add_argument("--timeout", type=float, default=15.0)
    parser.add_argument("--user-agent", default="academic-paper-skills-integrity/1.0")
    parser.add_argument("--report", type=Path, help="Optional JSON report path")
    parser.add_argument("--pretty", action="store_true")
    return parser


def main() -> int:
    args = build_parser().parse_args()
    try:
        ledger = json.loads(args.ledger.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        print(json.dumps({"decision": "BLOCKED", "errors": [f"cannot read ledger: {exc}"]}), file=sys.stderr)
        return 2
    report = validate_ledger(ledger, live=args.online, args=args)
    rendered = json.dumps(report, ensure_ascii=False, indent=2 if args.pretty else None)
    if args.report:
        args.report.parent.mkdir(parents=True, exist_ok=True)
        args.report.write_text(rendered + "\n", encoding="utf-8")
    print(rendered)
    return 0 if report["decision"] == "PASS" else 1


if __name__ == "__main__":
    raise SystemExit(main())
